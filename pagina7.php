...
$totalResult = $conexion->query("SELECT COUNT(*) AS total FROM costos");
$totalRows = $totalResult->fetch_assoc()['total'];
$totalPages = ceil($totalRows / $limit);

$result = $conexion->query("SELECT * FROM costos LIMIT $limit OFFSET $offset");

// Buscar
$searchResult = null;
if (isset($_POST['buscar'])) {
    $searchEmbarque = $_POST['searchEmbarque'];
    $searchPagoPersonal = $_POST['searchPagoPersonal'];
    $searchProveedores = $_POST['searchProveedores'];

    $stmt = $conexion->prepare("SELECT * FROM costos WHERE embarque LIKE ? AND pago_a_personal LIKE ? AND proveedores LIKE ?");
    $searchEmbarque = "%$searchEmbarque%";
    $searchPagoPersonal = "%$searchPagoPersonal%";
    $searchProveedores = "%$searchProveedores%";
    $stmt->bind_param("sss", $searchEmbarque, $searchPagoPersonal, $searchProveedores);
    $stmt->execute();
...
if (isset($_POST['crear'])) {
    ...
    $pago_a_personal = $_POST['pago_a_personal'];
    $proveedores = $_POST['proveedores'];
    $total = $_POST['total'];

    $stmt = $conexion->prepare("INSERT INTO costos (embarque, pago_a_personal, proveedores, total) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $embarque, $pago_a_personal, $proveedores, $total);
    $stmt->execute();
    $stmt->close();
}
...
if (isset($_POST['actualizar'])) {
    ...
    $stmt = $conexion->prepare("UPDATE costos SET embarque = ?, pago_a_personal = ?, proveedores = ?, total = ? WHERE id = ?");
    $stmt->bind_param("ssssi", $embarque, $pago_a_personal, $proveedores, $total, $id);
    $stmt->execute();
    $stmt->close();
}
...
if (isset($_POST['borrar'])) {
    ...
    $stmt = $conexion->prepare("DELETE FROM costos WHERE id = ?");
...
<body>
        ...
        <div class="container text-center">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="crearModalLabel">Crear Costo</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <form method="POST" action="" onsubmit="return confirmAction('crear')">
                                <div class="mb-3">
                                    <label for="embarque" class="form-label">Embarque</label>
                                    <input type="text" class="form-control" id="embarque" name="embarque" required>
                                </div>
                                <div class="mb-3">
                                    <label for="pago_a_personal" class="form-label">Pago a Personal</label>
                                    <input type="text" class="form-control" id="pago_a_personal" name="pago_a_personal" required>
                                </div>
                                <div class="mb-3">
                                    <label for="proveedores" class="form-label">Proveedores</label>
                                    <input type="text" class="form-control" id="proveedores" name="proveedores" required>
                                </div>
                                <div class="mb-3">
            ...
            <div class="modal fade" id="actualizarModal" tabindex="-1" aria-labelledby="actualizarModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="actualizarModalLabel">Actualizar Costo</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <form method="POST" action="" onsubmit="return confirmAction('actualizar')">
                                ...
                                <div class="mb-3">
                                    <label for="updateEmbarque" class="form-label">Embarque</label>
                                    <input type="text" class="form-control" id="updateEmbarque" name="embarque" required>
                                </div>
                                <div class="mb-3">
                                    <label for="updatePagoPersonal" class="form-label">Pago a Personal</label>
                                    <input type="text" class="form-control" id="updatePagoPersonal" name="pago_a_personal" required>
                                </div>
                                <div class="mb-3">
                                    <label for="updateProveedores" class="form-label">Proveedores</label>
                                    <input type="text" class="form-control" id="updateProveedores" name="proveedores" required>
                                </div>
                                <div class="mb-3">
                                    <label for="updateTotal" class="form-label">Total</label>
                                    <input type="text" class="form-control" id="updateTotal" name="total" required>
                                </div>
                            ...
                            <table class="table table-dark">
                                ...
                                <tbody>
                                    <?php if ($searchResult && $searchResult->num_rows > 0): ?>
                                        ...
                                        <tr>
                                            <td><?php echo $row['Embarque']; ?></td>
                                            <td><?php echo $row['Pago_a_personal']; ?></td>
            ...
            <form method="POST" action="" class="mt-5" onsubmit="return confirmAction('buscar')">
                <div class="row">
                    <div class="col-md-4">
                        <input type="text" class="form-control" name="searchEmbarque" placeholder="Buscar por Embarque">
                    </div>
                    <div class="col-md-4">
                        <input type="text" class="form-control" name="searchPagoPersonal" placeholder="Buscar por Pago a Personal">
                    </div>
                    <div class="col-md-4">
                        <input type="text" class="form-control" name="searchProveedores" placeholder="Buscar por Proveedores">
                    </div>
                    <div class="col-md-2">
            ...
            <h2 class="mt-5">Lista de Costos</h2>
            <table class="table table-dark">
                <thead>
                    <tr>
                        <th>Embarque</th>
    ...
    <script>
        function setUpdateData(id, embarque, pago_a_personal, proveedores, total) {
            document.getElementById('updateId').value = id;
            document.getElementById('updateEmbarque').value = embarque;
            document.getElementById('updatePagoPersonal').value = pago_a_personal;
            document.getElementById('updateProveedores').value = proveedores;
            document.getElementById('updateTotal').value = total;
        }

        function confirmAction(action) {
            return alertify.confirm("¿Estás seguro de que deseas " + action + " este costo?",
            function(){
